...
export const MonthBlock = memo(
  ({ year, month, habit, entries, notes, isFirst, onLongPress, colorKey }: MonthBlockProps) => {
    ...
    return (
      <div className="mb-6 [contain-intrinsic-size:auto_320px] [content-visibility:auto]">
        <div
          className={`flex justify-end border-t border-muted/10 px-4 py-3 ${isFirst ? 'border-t-0' : ''}`}
        >
          <span className={`text-[13px] ${tokens.text} font-display`}>{monthLabel}</span>
        </div>
